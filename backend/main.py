"""
Loan Origination Decision Engine — FastAPI Backend
Serves PD scoring, Expected Loss, SHAP explanations, and batch processing.
"""
...
class ApplicantInput(BaseModel):
    ...
    term:               int   = Field(..., ge=36, le=60, description="Loan term in months")
    ...
    sub_grade:          str   = Field(...,          description="Loan sub-grade e.g. A1")

    # Borrower profile
    annual_inc:         float = Field(..., gt=0,   description="Annual income ($)")
    emp_length:         Optional[float] = Field(None, description="Employment length (years)")
    home_ownership:     str   = Field(...,          description="RENT | MORTGAGE | OWN")
    verification_status:str   = Field(...,          description="Verified | Source Verified | Not Verified")
    application_type:   str   = Field("Individual", description="Individual | Joint App")

    # Credit bureau
    dti:                float = Field(..., ge=0,   description="Debt-to-income ratio (%)")
    fico_range_low:     float = Field(...,          description="FICO score low")
    fico_range_high:    float = Field(...,          description="FICO score high")
    open_acc:           Optional[float] = None
    pub_rec:            float = Field(0.0,          description="Public derogatory records")
    revol_bal:          Optional[float] = None
    revol_util:         Optional[float] = None
    total_acc:          Optional[float] = None
    pub_rec_bankruptcies: float = Field(0.0)
    mort_acc:           Optional[float] = None

    # Loan context
    purpose:            str   = Field(...,          description="Loan purpose")
    addr_state:         str   = Field("CA",         description="US state code")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "installment": 501.0, "grade": "C", "sub_grade": "C1",
                "annual_inc": 72000, "emp_length": 5.0,
                ...
                "open_acc": 8, "pub_rec": 0, "revol_bal": 12000,
                "revol_util": 45.0, "total_acc": 20,
                "pub_rec_bankruptcies": 0, "mort_acc": 0,
...
def build_dataframe(applicant: ApplicantInput) -> pd.DataFrame:
    ...
    # Feature engineering — mirrors notebook Section 6
    ...
    df["credit_history_year"] = 1998  # default; real app would take earliest_cr_line
...
def score_applicant(applicant: ApplicantInput):
    ...
    if bundle is None:
        # Demo mode — generate plausible synthetic score
        base = 0.05
...
@app.post("/score")
def score_single(applicant: ApplicantInput):
    """Score a single loan applicant — returns PD, EL, risk tier, SHAP reasons."""
    try:
...
async def score_batch(file: UploadFile = File(...)):
    ...
    Required columns match the ApplicantInput schema.
    Returns enriched CSV with pd_score, risk_tier, decision, expected_loss.
    """
    try:
...
@app.get("/")
def root():
    return {"message": "Loan Origination Decision Engine API v2.0", "docs": "/docs"}
